"""
AgentSkills + BAML demo — PLAIN DATA variant.

Skills are discovered from .agents/skills/ and passed to the LLM as a
list of {name, description} objects. The LLM returns a SkillSelection
struct with the chosen skill name (or null).

Demonstrates:
  - AgentSkills progressive disclosure (name/description → full instructions)
  - BAML structured routing with plain data types
...
def run():
    ...
    # Build the skill catalog as plain data — a list of {name, description}.
    skill_options = [
    ...
    while True:
        ...
        # ── Route ───────────────────────────────────────────────────────
        # Pass the skill list as data. The LLM returns a SkillSelection
        # with selected_skill (string) — we validate it against our dict.
        selection = b.SelectSkillPlain(query=query, skills=skill_options)
        ...
        print(f"  [skill: {skill.name} — {selection.reasoning}]")
